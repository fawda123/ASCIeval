...
load(file = 'C:/proj/ASCIeval/data/taxain.RData')
taxain <- taxain[, !names(taxain) %in% 'SampleID']
load(file = 'C:/proj/ASCIeval/data/sitein.RData')
sitein <- sitein[, !names(sitein) %in% 'SampleID']

# Step 1. Import taxonomy data -----------------------------------------------------------
bugs<- taxain #read.csv("demo_algae_tax.csv", stringsAsFactors = F) 
...
stations<-sitein # read.csv("demo_algae_sitedata.csv", stringsAsFactors = F)
...
# Import traits table 
traits<-read.csv('pMMI.input/combotraits.fromaaron4.csv',header=TRUE,strip.white=TRUE,check.names=FALSE)
...
d.win<-read.csv("pMMI.ouput/diatoms.combined.win.metrics.scaled.csv", row.names=1, stringsAsFactors = F)
sba.win<-read.csv("pMMI.ouput/sba.combined.win.metrics.scaled.csv", row.names=1, stringsAsFactors = F)
hybrid.win<-read.csv("pMMI.ouput/hybrid.combined.win.metrics.scaled.csv", row.names=1, stringsAsFactors = F)
