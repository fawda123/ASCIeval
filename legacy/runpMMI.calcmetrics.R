...
indicators.count=read.csv('pMMI.input/indic.with.count.csv',header=TRUE,strip.white=TRUE,check.names=FALSE)
...
stations_combined3 <- dplyr::left_join(stations_combined2, taxonomy_pa_melt, by = c('SampleID', 'FinalIDassigned', 'value'))
...
if (diatoms == T) { 
  ...
  filename<-paste0("pMMI.ouput/diatoms.models.out/", "diatoms.", "prop.spp.Salinity.BF", "RF.model.Rdata")
  load(filename)
